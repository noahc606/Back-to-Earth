...
RegTexBuilder::RegTexBuilder(Texture* tex, TileIterator& ti, int blitScale, TileType ttfc, int ttdfc)
{
	//Store blit's texture, size and destination
	RegTexBuilder::tex = tex;
	RegTexBuilder::blitScale = blitScale;
	RegTexBuilder::dstX = blitScale*ti.getTrackerSub(0);
	RegTexBuilder::dstY = blitScale*ti.getTrackerSub(1);
...
void RegTexBuilder::drawOverlay(int srcX, int srcY)
{
	int sx32 = (srcX*32);
	int sy32 = (srcY*32);
