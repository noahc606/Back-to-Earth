        ...
        if( w==1 ) {
            ...
            switch(cmdID) {
                case CMD_debug: {
                    ...
                    if(s>3) {
                        nch::Log::log( errorTooManyArgs(3, commandList[cmdID]) );
                        return -1;
                    }
                } break;
                case CMD_tele: {
                    ...
                    if(s==2) {
                        nch::Log::log( errorMissingArg(3, args[0]) );
                        return -1;
                    }
                } break;
            }
            ...
            switch(cmdID)
            {
                case CMD_kv: {
                    ...
                    if(s==2) {
                        int x = cmdKV(word);
                        if( x!=0 ) {
                            nch::Log::log(errorMissingArg(3, commandList[cmdID]));
                            return -1;
                        }
                    }
                } break;
                ...
                case CMD_debug: {
                    if( word=="\"memTexes\"" ) {
                        ...
                        nch::Log::log("Unknown argument '"+word+"'...");
                    }
                } break;
                case CMD_gs: {
                    if( parseString(word)==ParseTypes::INT ) {
                        ...
                        nch::Log::log(errorInvalidArg(commandList[cmdID], 2, "integer"));
                        return -1;
                    }
                } break;
                case CMD_tele: {
                ...
                case CMD_debug: {
                    ...
                    if( args[1]=="\"memTexes\"" ) {
                        if( parseString(word)==ParseTypes::INT ) {
                            ...
                            nch::Log::log(errorInvalidArg(commandList[cmdID], 3, "integer"));
                        }
                    }
                } break;
                case CMD_tele: {
            ...
            switch(cmdID) {
                case CMD_tele: {
                    ...
                    if( parseString(word)==ParseTypes::INT ) {
                        ...
                        nch::Log::log(errorInvalidArg(commandList[cmdID], 4, "integer"));
                        return -1;
                    }
                }
            }
...
int Commands::resetCMDEntered(std::string methodName)
{
    if( getInt("~cmd.state")==nullptr &&
        ...
        nch::Log::warn(methodName, "Tried to reset key \"~cmd.state\" which doesn't exist");
        return -1;
    }
...
void Commands::ssWords(const std::vector<std::string>& words)
{
    ...
    wordsFormatted << "}";
    nch::Log::log(wordsFormatted.str());
}
